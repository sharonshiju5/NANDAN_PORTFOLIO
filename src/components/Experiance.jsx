...
export default function Experiance() {
    const [currentView, setCurrentView] = useState(0); // 0 for single card, 1 for three cards
        ...
        <div className=" min-h-screen p-8">
            <div className="max-w-6xl mx-auto">
                    ...
                    {currentView === 0 ? (
                        ...
                        >
                            ...
                            <div className=" relative h-[500px] text-white p-8 rounded-3xl flex-1 max-w-md">
                                <img className='absolute left-0 top-0 -z-10 w-full h-full' src={image1} alt="" />

                                <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center mb-6">
                                    <img src={logo1} alt="" />
                                </div>
                                
                                <h3 className="text-2xl font-bold mb-8">{experiences[currentIndex].company}</h3>
                                ...
                                <div className="mt-auto">
                                    <p className="text-lg font-semibold">{experiences[currentIndex].period}</p>
                                    <p className="text-base opacity-90">{experiences[currentIndex].location}</p>
                                </div>
                            </div>
                            
                            {/* CTA Section */}
                            <div  className=" text-white h-[500px] p-8 rounded-3xl flex-1 relative overflow-hidden">
                                <img className='absolute -z-10 top-0 left-0 w-full h-full' src={image2} alt="" />
                        ...
                        >
                            {allExperiences.map((exp, index) => (
                                ...
                                >
                                <img className='absolute -z-10 top-0 left-0 w-full h-full' src={exp.image} alt="" />

                                    <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center mb-6">
                                            <img src={exp.logo} alt="" />
                                        {/* <div className="w-8 h-8 bg-gray-800 rounded-full flex items-center justify-center">
                                            <span className="text-white font-bold text-sm">{exp.company[0]}hgvj</span>
                                        </div> */}
                                    </div>
                ...
                <div className="flex justify-end gap-4 mt-8">
                    <button 
                        onClick={prevSlide}
                        className="w-12 h-12 border border-gray-300 text-white rounded-full flex items-center justify-center hover:bg-gray-800 transition-colors"
                    >
                        ←
                    </button>
                    <button 
